...
class AveragedSelector(object):
    ...
    def __init__(self, trials = 1, T = 10, number_of_actions = 2):
        ...
        self.control_probability = np.zeros((trials, self.na))
    ...
    def select_desired_action(self, tau, posterior_policies, actions, *args):
        
        #estimate action probability
        self.estimate_action_probability(tau, posterior_policies, actions)
        
        #generate the desired response from action probability
        print(posterior_policies)
        u = np.random.choice(self.na, p = posterior_policies)
        
        return u
    
    def estimate_action_probability(self, tau, posterior_policies, actions, *args):

            
        self.control_probability[tau] = posterior_policies
